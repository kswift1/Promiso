...
///
/// Firestore의 "날짜" 필드(series_start_date, excluded_dates 등)는 사용자가 KST 기준으로
/// 입력한 값이므로, 전체 timestamp로 저장되어 있어도 반드시 KST로 변환한 뒤 날짜를 추출해야
/// 한다. UTC date_naive() 를 쓰면 KST 자정 근처 데이터가 하루 밀리는 버그가 발생.
