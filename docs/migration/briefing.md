...
| 클라이언트 | 현재 경로 | 상태 |
|---|---|---|
| `BriefingClient.generate` | Rust briefing API | ✅ release도 Rust 고정 |

메모:

- `Projects/Clients/Sources/Clients/FeatureFlagsClient.swift`에서 `briefing` 도메인은 release에서도 Rust를 강제한다.
...
- iOS:
  - `FeatureFlagsClientTests.swift`에 release briefing routing 테스트 추가
