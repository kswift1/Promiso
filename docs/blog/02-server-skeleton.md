...
Railway는 git push만 하면 배포가 끝나서 DX가 압도적이다. 솔직히 끌렸다. 하지만 서울 리전이 없다는 게 발목을 잡았다. Rust 콜드스타트가 100ms 이내라 Cloud Run의 scale-to-zero도 부담이 아니었고, 결국 **서울 리전 + 비용 0**인 Cloud Run을 선택했다.
...
Cloud SQL은 Cloud Run과 같은 GCP 프로젝트 안에서 VPC로 연결된다. 서버-DB 레이턴시가 약 1ms. 자동 백업, 자동 패치. Neon의 Dev 환경에서 70ms가 붙어도 개발할 때는 체감이 안 된다.
...
스토리지도 마찬가지다. iOS가 Firebase Storage에 이미지를 올리고, Rust에는 경로만 알려주면 된다.
...
결정이 끝났으니 실제로 코드를 만들 차례다. Rust 웹 프레임워크로 Axum, DB 라이브러리로 SQLx를 선택했다. 둘 다 Rust 생태계에서 가장 널리 쓰이는 조합이다.
...
health check 엔드포인트 하나로 전체가 연결되는지 확인했다:
...
Swift의 `Error` enum과 비슷하다. 각 variant가 HTTP 상태 코드와 Firebase 에러 코드에 매핑된다. Internal 에러만 클라이언트에 상세 메시지를 숨기고 로그에만 기록한다.
...
결국 Mac의 LAN IP(`192.168.x.x`)를 직접 넣으니 연결됐다. iOS 26 시뮬레이터가 loopback 인터페이스를 제대로 못 타는 것 같다. 깔끔한 해결은 아니지만, 로컬 개발 단계에서는 이걸로 충분하다. Cloud Run에 배포하면 어차피 실제 URL을 쓰게 된다.
