...
근데 수준이 달랐다. 1편에서 겪은 Firebase Functions의 콜드스타트는 1~5초였다. Cloud Run 공식 문서도 Node.js 같은 동적 언어는 모듈 로딩이 startup latency에 더해진다고 설명한다. Rust는 네이티브 바이너리라 그 오버헤드가 상대적으로 적다. 내 컨테이너 기준으로는 첫 요청 추가 지연이 100ms 안쪽이었다.

콜드스타트를 없앨 방법도 있었다. `min-instances=1`로 warm instance를 항상 유지하면 된다. Firebase Functions에도 같은 옵션이 있었는데, 함수마다 비용이 붙어서 50개가 넘으면 부담이 급증했다. Cloud Run은 서비스 단위로 관리하니 비용 구조가 단순하다. 하지만 지금은 트래픽이 없는 초기 단계라 굳이 쓸 이유가 없다. 트래픽이 생기면 그때 올리면 된다.
