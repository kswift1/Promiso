...
//  Clients
//
//  ScheduleClient liveValue 어댑터 로직 및 에러 매핑 테스트
//
//  ## 테스트 대상
//  - `Clients/Sources/Clients/ScheduleClient.swift`
//
//  ## 테스트 목적
//  - ScheduleClientError 에러 매핑 검증 (NSError → ScheduleClientError)
//  - ScheduleClientError localizedDescription 검증
//  - createSchedule 빈 groupId 검증
//  - ScheduleClientError Equatable 동작 검증
//
...
// MARK: - ScheduleClientError 에러 매핑 테스트

@Suite("ScheduleClientError 에러 매핑 테스트")
struct ScheduleClientErrorMappingTests {
  ...
  func nonFunctionsDomain_mapsToNetworkError() {
    ...
    let error = ScheduleClientError(from: nsError)
  ...
  func functionsDomain_code3And9_mapsToInvalidData() {
    ...
    let invalidArg = ScheduleClientError(from: invalidArgError)
    ...
    let failedPrecondition = ScheduleClientError(from: failedPreconditionError)
  ...
  func functionsDomain_variousCodes_mapCorrectly() {
    ...
    #expect(ScheduleClientError(from: notFoundError) == .groupNotFound)
    ...
    #expect(ScheduleClientError(from: notMemberError) == .notGroupMember)
    ...
    #expect(ScheduleClientError(from: serverError) == .serverError)
  }
  ...
  func functionsDomain_unknownCode_mapsToUnknown() {
    ...
    let error = ScheduleClientError(from: unknownError)
    #expect(error == .unknown("알 수 없는 에러"))
  }
}

// MARK: - ScheduleClientError localizedDescription 테스트

@Suite("ScheduleClientError localizedDescription 테스트")
struct ScheduleClientErrorDescriptionTests {
  ...
  func invalidData_customMessage_returnsCustomMessage() {
    let error = ScheduleClientError.invalidData("제목은 필수입니다")
  ...
  func unknown_customMessage_returnsCustomMessage() {
    let error = ScheduleClientError.unknown("timeout 발생")
