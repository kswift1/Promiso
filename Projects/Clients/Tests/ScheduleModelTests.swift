...
// MARK: - ScheduleModel Computed Properties 테스트

@Suite("ScheduleModel computed property 테스트")
struct ScheduleModelComputedTests {
  ...
  func isConfirmed_whenAcceptedMeetsMinimum_returnsTrue() {
    ...
    let schedule = TestFactories.makeSchedule(minimumParticipants: 2, votes: votes)
    #expect(schedule.isConfirmed == true)
  }
  ...
  func isConfirmed_whenAcceptedBelowMinimum_returnsFalse() {
    ...
    let schedule = TestFactories.makeSchedule(minimumParticipants: 2, votes: votes)
    #expect(schedule.isConfirmed == false)
  }
  ...
  func isHost_whenUserIdMatchesHostId_returnsTrue() {
    let schedule = TestFactories.makeSchedule(hostId: "host-user")
    #expect(schedule.isHost(userId: "host-user") == true)
  }

  @Test("[P10] 호스트 ID와 불일치하면 호스트 아님")
  func isHost_whenUserIdDoesNotMatch_returnsFalse() {
    let schedule = TestFactories.makeSchedule(hostId: "host-user")
    #expect(schedule.isHost(userId: "other-user") == false)
  }
  ...
  func myVoteStatus_whenInAccepted_returnsAccepted() {
    ...
    #expect(schedule.myVoteStatus(userId: "user1") == .accepted)
  }
  ...
  func myVoteStatus_whenInDeclined_returnsDeclined() {
    ...
    let schedule = TestFactories.makeSchedule(votes: votes)
    #expect(schedule.myVoteStatus(userId: "user1") == .declined)
  }
  ...
  func myVoteStatus_whenNotInAnyList_returnsPending() {
    ...
    let schedule = TestFactories.makeSchedule(votes: votes)
    #expect(schedule.myVoteStatus(userId: "user3") == .pending)
  }
  ...
  func isVotingClosed_whenDeadlinePassed_returnsTrue() {
    ...
    let schedule = TestFactories.makeSchedule(votes: votes)
    #expect(schedule.isVotingClosed == true)
  }
  ...
  func isVotingClosed_whenDeadlineInFuture_returnsFalse() {
    ...
    let schedule = TestFactories.makeSchedule(votes: votes)
    #expect(schedule.isVotingClosed == false)
  }
}

// MARK: - ScheduleModel Time-based Properties 테스트

@Suite("ScheduleModel 시간 기반 속성 테스트")
struct ScheduleModelTimeTests {
  ...
  func isOngoing_whenBetweenStartAndEnd_returnsTrue() {
    ...
    let schedule = TestFactories.makeSchedule(startAt: startAt, endAt: endAt)
    #expect(schedule.isOngoing == true)
  }
  ...
  func isOngoing_whenBeforeStart_returnsFalse() {
    ...
    let schedule = TestFactories.makeSchedule(startAt: startAt, endAt: endAt)
    #expect(schedule.isOngoing == false)
  }
  ...
  func isOngoing_whenNoEndAtAndAfterStart_returnsTrue() {
    let startAt = Date().addingTimeInterval(-3600)
    let schedule = TestFactories.makeSchedule(startAt: startAt, endAt: nil)
    #expect(schedule.isOngoing == true)
  }
  ...
  @Test("[P35] 종료 시간 없고 시작 시간 지났으면 과거 일정")
  ...
  @Test("[P35] 시작 시간이 미래면 과거 일정 아님")
  func isPast_whenStartInFuture_returnsFalse() {
    let startAt = Date().addingTimeInterval(3600)
    let schedule = TestFactories.makeSchedule(startAt: startAt)
    #expect(schedule.isPast == false)
  }

  // MARK: - isUpcoming 테스트

  @Test("[P36] 시작 시간이 미래면 다가오는 일정")
  ...
  @Test("[P36] 시작 시간이 과거면 다가오는 일정 아님")
  ...
  func isRealtimeShareable_whenNoTracking_returnsFalse() {
    let schedule = TestFactories.makeSchedule(
      startAt: Date().addingTimeInterval(600),
      trackingStartMinutesBefore: nil
    )
    #expect(schedule.isRealtimeShareable == false)
  }
  ...
  func isRealtimeShareable_whenWithinTrackingWindow_returnsTrue() {
    ...
    #expect(schedule.isRealtimeShareable == true)
  }
...
// MARK: - ScheduleModel UI Display 테스트

@Suite("ScheduleModel UI 표시 속성 테스트")
struct ScheduleModelDisplayTests {

  // MARK: - displayEmoji 테스트

  @Test("[P20] 이모지가 있으면 해당 이모지 반환")
  func displayEmoji_whenEmojiSet_returnsEmoji() {
    let schedule = TestFactories.makeSchedule(emoji: "🎉")
    #expect(schedule.displayEmoji == "🎉")
  }

  @Test("[P20] 이모지가 nil이면 기본 이모지 반환")
  func displayEmoji_whenEmojiNil_returnsDefault() {
    let schedule = TestFactories.makeSchedule(emoji: nil)
    #expect(schedule.displayEmoji == "📌")
  }
  ...
  func locationText_whenLocationSet_returnsName() {
    ...
    let schedule = TestFactories.makeSchedule(location: location)
    #expect(schedule.locationText == "강남역")
  }
  ...
  func dateText_whenToday_returnsToday() {
    // 오늘 시간으로 일정 생성
    let schedule = TestFactories.makeSchedule(startAt: Date())
    #expect(schedule.dateText == LocalizedStrings.DateFormat.today)
  }

  @Test("내일 날짜면 '내일' 반환")
  func dateText_whenTomorrow_returnsTomorrow() {
    let tomorrow = Calendar.current.date(byAdding: .day, value: 1, to: Date())!
    let schedule = TestFactories.makeSchedule(startAt: tomorrow)
    #expect(schedule.dateText == LocalizedStrings.DateFormat.tomorrow)
  }
  ...
  func dateText_whenOtherDate_returnsFormattedDate() {
    ...
    let schedule = TestFactories.makeSchedule(startAt: futureDate)

    let expected = LocalizedDateFormatters.monthDayString(from: futureDate)

    #expect(schedule.dateText == expected)
  }
  ...
  func timeRangeText_whenNoEndAt_returnsTimeTextOnly() {
    let schedule = TestFactories.makeSchedule(endAt: nil)
    #expect(schedule.timeRangeText == schedule.timeText)
  }
  ...
  func deadlineText_whenPastDeadline_returnsNil() {
    let votes = TestFactories.makeVotes(until: Date().addingTimeInterval(-3600))
    let schedule = TestFactories.makeSchedule(votes: votes)
    #expect(schedule.deadlineText == nil)
  }
  ...
  func shareText_containsBasicInfo() {
    ...
    #expect(schedule.shareText.contains("🍕"))
    #expect(schedule.shareText.contains("점심 모임"))
    #expect(schedule.shareText.contains("📅"))
  }
  ...
  func shareText_withLocation_containsLocation() {
    ...
    #expect(schedule.shareText.contains("📍"))
    #expect(schedule.shareText.contains("강남역"))
  }
  ...
  func shareText_withDescription_containsDescription() {
    ...
    #expect(schedule.shareText.contains("월간 정기 회의"))
  }
  ...
  func empty_hasEmptyTitle() {
    let schedule = ScheduleModel.empty
    #expect(schedule.title == "")
  }
}

// MARK: - ScheduleModel Response Status 테스트

@Suite("ScheduleModel responseStatus 테스트")
struct ScheduleModelResponseStatusTests {
  ...
  func responseStatus_whenVotingClosedAndNotConfirmed_returnsFailed() {
    ...
    let schedule = TestFactories.makeSchedule(minimumParticipants: 3, votes: votes)
    let status = schedule.responseStatus(currentUserId: "user2")
  ...
  func responseStatus_whenAllRespondedAndNotConfirmed_returnsFailed() {
    ...
    let schedule = TestFactories.makeSchedule(minimumParticipants: 3, votes: votes)
    let status = schedule.responseStatus(currentUserId: "user1", totalGroupMembers: 3)
    #expect(status == .failed)
  }
  ...
  func responseStatus_whenCannotReachMinimum_returnsFailed() {
    ...
    let schedule = TestFactories.makeSchedule(minimumParticipants: 3, votes: votes)
    let status = schedule.responseStatus(currentUserId: "user3", totalGroupMembers: 4)
    #expect(status == .failed)
  }
  ...
  @Test("[P26] 내가 응답했고 일정이 확정되면 confirmed")
  func responseStatus_whenRespondedAndConfirmed_returnsConfirmed() {
    ...
    let schedule = TestFactories.makeSchedule(minimumParticipants: 2, votes: votes)
    let status = schedule.responseStatus(currentUserId: "user1")
  ...
  func responseStatus_whenNilUserAndConfirmed_returnsConfirmed() {
    ...
    let schedule = TestFactories.makeSchedule(minimumParticipants: 2, votes: votes)
    let status = schedule.responseStatus(currentUserId: nil)
    #expect(status == .confirmed)
  }
...
// MARK: - ScheduleResponseStatus / ScheduleAttendanceStatus 테스트

@Suite("ScheduleResponseStatus 및 ScheduleAttendanceStatus 테스트")
struct ScheduleStatusEnumTests {

  @Test("ScheduleResponseStatus rawValue 올바름")
  func responseStatus_rawValues() {
    #expect(ScheduleResponseStatus.needResponse.rawValue == "needResponse")
    #expect(ScheduleResponseStatus.responded.rawValue == "responded")
    #expect(ScheduleResponseStatus.confirmed.rawValue == "confirmed")
    #expect(ScheduleResponseStatus.failed.rawValue == "failed")
  }

  @Test("ScheduleAttendanceStatus rawValue 올바름")
  func attendanceStatus_rawValues() {
    #expect(ScheduleAttendanceStatus.pending.rawValue == "pending")
    #expect(ScheduleAttendanceStatus.accepted.rawValue == "accepted")
    #expect(ScheduleAttendanceStatus.declined.rawValue == "declined")
  }
}
