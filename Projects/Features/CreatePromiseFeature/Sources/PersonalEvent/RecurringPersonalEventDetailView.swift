...
extension RecurringPersonalEventDetail {
  public struct RootView: View {
    ...
    private var recurrenceSection: some View {
      VStack(spacing: 0) {
        PromiseDetailSectionHeader(title: "반복 규칙")
    ...
    private var locationSection: some View {
      VStack(spacing: 0) {
        PromiseDetailSectionHeader(title: LocalizedStrings.Common.location)

        if let location = store.recurringEvent.location {
          PromiseDetailLocationInfoRow(location: location)
            .adaptiveGlassCard()
        }
      }
    }
    ...
    private var descriptionSection: some View {
      VStack(spacing: 0) {
        PromiseDetailSectionHeader(title: "메모")
