// MARK: - ProPlanFeatureExampleApp.swift
// ProPlan Feature 개발과 테스트를 위한 독립 실행형 example 애플리케이션
...
/// ProPlan Feature를 위한 독립 실행형 example 앱
/// 다양한 테스트 시나리오와 함께 격리된 개발 환경을 제공
@main
struct ProPlanFeatureExampleApp: App {
  
  var body: some Scene {
    WindowGroup {
      NavigationStack {
        ExampleContentView()
          .navigationTitle("ProPlan Examples")
...
private struct ExampleContentView: View {
  ...
  private var defaultExample: some View {
    let store = Store(initialState: ProPlan.Feature.State()) {
      ProPlan.Feature()
    }
