// MARK: - PersonalFeatureExampleApp.swift
// Personal Feature 개발과 테스트를 위한 독립 실행형 example 애플리케이션
...
/// Personal Feature를 위한 독립 실행형 example 앱
...
struct PersonalFeatureExampleApp: App {
  ...
  var body: some Scene {
    WindowGroup {
      NavigationStack {
        ExampleContentView()
          .navigationTitle("Personal Examples")
...
private struct ExampleContentView: View {
  ...
  private var defaultExample: some View {
    let store = Store(initialState: Personal.Feature.State()) {
      Personal.Feature()
    }
    
    Personal.RootView(store: store)
  }
}
