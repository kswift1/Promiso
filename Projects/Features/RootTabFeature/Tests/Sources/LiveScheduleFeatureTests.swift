...
@Suite("LiveSchedule.Feature reducer 테스트")
@MainActor
struct LiveScheduleFeatureTests {

  // MARK: - 초기 상태 테스트

  @Test("초기 상태 기본값 확인")
  func initialState_hasCorrectDefaults() {
    let state = LiveSchedule.Feature.State(
      emoji: "📍",
      title: "일정",
      currentUserId: "test-user"
    )
    #expect(state.data.emoji == "📍")
    #expect(state.data.title == "일정")
  ...
  func processPendingETAUpdate_withNoActivityId_clearsProcessing() async {
    let data = makeLiveData(isProcessingETAUpdate: true)
    var state = LiveSchedule.Feature.State(data: Shared(value: data))
    let etaUpdate = ETAUpdate(scheduleId: "", userId: "test-user", estimatedMinutes: 10, timestamp: Date())
  ...
  func etaUpdateSent_clearsStateAndRefreshes() async {
    let data = makeLiveData(isProcessingETAUpdate: true)
    var state = LiveSchedule.Feature.State(data: Shared(value: data))
    state.pendingETAUpdate = ETAUpdate(scheduleId: "", userId: "test-user", estimatedMinutes: 10, timestamp: Date())
  ...
  func etaUpdateFailed_clearsState() async {
    let data = makeLiveData(isProcessingETAUpdate: true)
    var state = LiveSchedule.Feature.State(data: Shared(value: data))
    state.pendingETAUpdate = ETAUpdate(scheduleId: "", userId: "test-user", estimatedMinutes: 5, timestamp: Date())
  ...
  @Test("observeContentStateUpdates 활성 일정 없으면 아무 동작 안 함")
  func observeContentStateUpdates_withoutActiveSchedule_doesNothing() async {
    let store = makeStore()
    await store.send(.internal(.observeContentStateUpdates))
  }

  @Test("observeContentStateUpdates 활성 일정이면 스트림 구독")
  func observeContentStateUpdates_withActiveSchedule_subscribes() async {
    let newContentState = ScheduleActivityAttributes.ContentState(
    ...
    let store = makeStore() {
      $0.liveActivityClient.activeScheduleId = { "schedule-1" }
  ...
  func contentStateUpdated_syncsParticipants() async {
    let updatedContentState = ScheduleActivityAttributes.ContentState(
...
private extension LiveScheduleFeatureTests {
  ...
  ) -> TestStoreOf<LiveSchedule.Feature> {
    let resolvedData = data ?? makeLiveData()
    return TestStore(initialState: LiveSchedule.Feature.State(data: Shared(value: resolvedData))) {
      LiveSchedule.Feature()
    } withDependencies: {
      applyDefaultDependencies(&$0)
      configure(&$0)
  ...
  func applyDefaultDependencies(_ deps: inout DependencyValues) {
    ...
    deps.liveActivityClient.activeScheduleId = { nil }
