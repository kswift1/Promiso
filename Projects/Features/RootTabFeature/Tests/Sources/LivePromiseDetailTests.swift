...
@Suite("LivePromise.Detail reducer 테스트")
@MainActor
struct LivePromiseDetailTests {
  ...
  func submitCustomMinute_valid_sendsDelegate() async {
    let data = makeLiveData()
    var state = LivePromise.Detail.State(data: Shared(value: data))
...
private extension LivePromiseDetailTests {
  ...
  ) -> TestStoreOf<LivePromise.Detail> {
    let resolvedData = data ?? makeLiveData()
    return TestStore(
      initialState: LivePromise.Detail.State(data: Shared(value: resolvedData))
    ) {
      LivePromise.Detail()
    } withDependencies: {
      applyDefaultDependencies(&$0)
