//
//  LiveScheduleExpandedView.swift
//  RootTabFeature
//
...
//            → LiveScheduleFeature.contentStateUpdated 액션
//            → LiveSchedule.Data (@Shared) 업데이트
//            → ExpandedView 자동 갱신
//
// ┌─────────────────────────────────────────────────────────────────────────────┐
...
// └─────────────────────────────────────────────────────────────────────────────┘
//
// • ScheduleActivityAttributes (고정 정보)
//   - scheduleId, currentUserId, emoji, title, location, scheduledTime
...
// MARK: - LiveSchedule.ExpandedView

extension LiveSchedule {
  /// 일정 추적 확장 뷰 (전체 화면) - DetailSpec v5 기준
  ...
  public struct ExpandedView: View {
    ...
    // MARK: - Computed Properties for Header (ScheduleCard 동일)

    /// 일정 모델 (store에서 가져오거나 fallback)
    private var schedule: ScheduleModel? {
      store.schedule
    }

    /// 호스트 정보 (ScheduleModel 기반 - fallback)
    private var host: UserPublicModel? {
      guard let schedule = schedule else { return nil }
      return store.groupMembers?.first { $0.userId == schedule.hostId }
    }
    ...
    private var isHost: Bool {
      ...
      // Fallback: ScheduleModel 기반
      guard let schedule = schedule else { return false }
      return schedule.isHost(userId: store.currentUserId)
    }
    ...
    private var groupName: String? {
      store.data.groupName ?? schedule?.group?.name
    }

    /// 그룹 이미지 URL (ActivityKit에서 가져오기)
    private var groupImageUrl: String? {
      store.data.groupImageUrl ?? schedule?.group?.imageUrl
    }
      ...
      VStack(alignment: .leading, spacing: 14) {
        ...
        HStack(spacing: 10) {
          GroupThumbnailView(
            ...
            name: groupName ?? LocalizedStrings.LiveSchedule.groupFallback,
            size: 32
          )

          Text(groupName ?? LocalizedStrings.LiveSchedule.groupFallback)
        ...
        // 2. Main Content (ScheduleCard와 동일)
        HStack(alignment: .top, spacing: 12) {
          Text(schedule?.displayEmoji ?? store.data.emoji)
            .font(.system(size: 44))

          VStack(alignment: .leading, spacing: 10) {
            Text(schedule?.title ?? store.data.title)
              .font(.system(size: 19, weight: .bold))
              .foregroundColor(.primary)

            // Description (ScheduleCard와 동일)
            ...
            VStack(alignment: .leading, spacing: 6) {
              // 시간 (ScheduleCard와 동일)
              if let dateText = schedule?.dateText, let timeText = schedule?.timeText {
                HStack(spacing: 4) {
                  Text("⏰")
                    .font(.system(size: 14))
                ...
                // Fallback: LiveSchedule.Data에서 가져오기
                HStack(spacing: 4) {
                  Text("⏰")
                    .font(.system(size: 14))
              ...
              // 장소 (ScheduleCard와 동일 - "장소 미정"이면 숨김)
              if !isLocationUndecided {
                if let locationText = schedule?.locationText {
                  HStack(spacing: 4) {
                    Text("📍")
                      .font(.system(size: 14))
                    Text(locationText)
    ...
    private func remainingTimeView(until date: Date) -> some View {
      ...
      return HStack(spacing: 4) {
        ...
        if hours > 0 {
          Text(LocalizedStrings.LiveSchedule.hoursMinutes(hours, minutes))
            .font(.title3.weight(.bold).monospacedDigit())
        } else {
          Text(LocalizedStrings.LiveSchedule.minutesOnly(minutes))
            .font(.title3.weight(.bold).monospacedDigit())
        }

        if isOverdue {
          Text(LocalizedStrings.LiveSchedule.elapsed)
    ...
    private var liveBadge: some View {
      HStack(spacing: 4) {
        ...
        Text(LocalizedStrings.LiveSchedule.realtime)
    ...
    private var directionsButton: some View {
      Button {
        ...
        HStack(spacing: 8) {
          ...
          Text(LocalizedStrings.LiveSchedule.directions)
    ...
    private var participantStatusHeader: some View {
      HStack {
        HStack(spacing: 6) {
          ...
          Text(LocalizedStrings.LiveSchedule.participantsStatus)
        ...
        Text(LocalizedStrings.LiveSchedule.arrivedCount(store.data.arrivedCount, store.data.participants.count))
    ...
    private var participantNotice: some View {
      VStack(alignment: .leading, spacing: 2) {
        Text(LocalizedStrings.LiveSchedule.etaNotice1)
        Text(LocalizedStrings.LiveSchedule.etaNotice2)
      }
      .font(.caption2)
      ...
      let rowContent = HStack(spacing: 12) {
        ...
        VStack(alignment: .leading, spacing: 2) {
          HStack(spacing: 6) {
            ...
            if isCurrentUser {
              Text(LocalizedStrings.LiveSchedule.me)
        ...
        VStack(alignment: .trailing, spacing: 4) {
          ...
          if isCurrentUser {
            Text(LocalizedStrings.LiveSchedule.changeAction)
              .font(.caption.weight(.medium))
              .foregroundStyle(secondaryTextColor)
          }
        }
        ...
        if let eta = participant.estimatedArrivalMinutes {
          if eta == 0 {
            ...
            HStack(spacing: 4) {
              ...
              Text(LocalizedStrings.LiveSchedule.arrived)
                .font(.subheadline.weight(.semibold))
            }
          ...
          // 대기
          Text(LocalizedStrings.LiveSchedule.waiting)
            .font(.subheadline)
            .foregroundStyle(secondaryTextColor)
            .padding(.horizontal, 12)
            .padding(.vertical, 6)
    ...
    private var etaChangeSheet: some View {
      VStack(spacing: 24) {
        ...
        VStack(spacing: 12) {
          HStack(spacing: 12) {
            etaSheetButton(title: LocalizedStrings.LiveSchedule.etaArrived, emoji: "🏁", minutes: 0, color: Color.pmsuccess.n500)
            etaSheetButton(title: LocalizedStrings.LiveSchedule.etaMinutes(5), emoji: nil, minutes: 5, color: Color.pmindigo.n500)
          }

          HStack(spacing: 12) {
            etaSheetButton(title: LocalizedStrings.LiveSchedule.etaMinutes(10), emoji: nil, minutes: 10, color: Color.pmindigo.n500)
            etaSheetButton(title: LocalizedStrings.LiveSchedule.etaMinutes(15), emoji: nil, minutes: 15, color: Color.pmindigo.n500)
          }

          HStack(spacing: 12) {
            etaSheetButton(title: LocalizedStrings.LiveSchedule.etaMinutes(20), emoji: nil, minutes: 20, color: Color.pmwarning.n500)
            etaSheetButton(title: LocalizedStrings.LiveSchedule.etaMinutes(30), emoji: nil, minutes: 30, color: Color.pmerror.n500)
          }
        }
        .padding(.top, 12)
        ...
        // 직접 입력
        VStack(spacing: 8) {
          Text(LocalizedStrings.LiveSchedule.directInput)
            .font(.subheadline.weight(.medium))
            .foregroundStyle(secondaryTextColor)

          HStack(spacing: 12) {
            TextField(LocalizedStrings.LiveSchedule.minuteUnit, text: $store.customMinuteInput.sending(\.view.customMinuteInputChanged))
            ...
            Button {
              ...
              Text(LocalizedStrings.LiveSchedule.confirm)
                .font(.system(size: 15, weight: .semibold))
                .foregroundStyle(.white)
                .padding(.horizontal, 20)
                .padding(.vertical, 12)
