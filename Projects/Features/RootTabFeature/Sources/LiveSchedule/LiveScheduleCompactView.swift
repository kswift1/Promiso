...
extension LiveSchedule {
  /// 하단 컴팩트 뷰 (일정 추적 바) - tabViewBottomAccessory / overlay에서 사용
      ...
      HStack(spacing: 10) {
        ...
        VStack(alignment: .leading, spacing: 2) {
          ...
          HStack(spacing: 3) {
            ...
            Text(LocalizedStrings.LiveSchedule.participantCount(store.data.participants.count))
              .font(.caption2)
          }
...
#Preview("LiveSchedule Compact - Light") {
  VStack {
    LiveSchedule.CompactView(
      store: Store(
        initialState: LiveSchedule.Feature.State(
          emoji: "🍜",
          title: "점심 모임",
          location: "강남역 11번 출구",
...
#Preview("LiveSchedule Compact - Dark") {
  VStack {
    LiveSchedule.CompactView(
      store: Store(
        initialState: LiveSchedule.Feature.State(
