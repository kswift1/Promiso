...
## 개요
그룹의 일정을 조회하고 관리하는 Group Feature의 화면 렌더링 플로우를 설명합니다.
...
그룹 선택 우선순위:
  ...
  ↓
currentGroup == nil?
  ├─ YES: schedulesState = .loaded([])
  │       화면: GroupDetailEmptyView
  │
  └─ NO: .internal(.fetchSchedules(groupId)) 전송
```
...
### 5단계: Internal.fetchSchedules

```
Internal.fetchSchedules(groupId)
  ↓
schedulesState = .loading
  ↓
Effect: scheduleClient.fetchSchedules(groupId)
  ↓
  ├─ Success → .internal(.loadSchedulesResponse(.success(schedules)))
  └─ Failure → .internal(.loadSchedulesResponse(.failure(error)))
```
...
### 6단계: Internal.loadSchedulesResponse

```
Internal.loadSchedulesResponse
  ↓
  ├─ [Success]
  │    ↓
  │    schedulesState = .loaded(schedules)
  │    ↓
  │    화면: 정상 일정 리스트
  │      ├─ StatusFilterView
...
RootView.body
  ↓
  ├─ [조건 1] schedulesState == .idle || .loading
  │    ↓
  │    ProgressView (로딩 스피너)
  │
  └─ [조건 2] schedulesState == .loaded || .failed
       ↓
       ...
       └─ shouldShowEmptyGroupView == false
            ...
            mainContentView
              ...
              │
              └─ ScheduleTimelineView
                   ├─ .loaded(schedules) → 일정 리스트
                   └─ .failed(error) → 에러 메시지
```

### 화면 상태 정리

| schedulesState | shouldShowEmptyGroupView | 화면 |
|--------------|-------------------------|------|
| `.idle` | - | 로딩 스피너 |
| `.loading` | - | 로딩 스피너 |
| `.loaded([])` | `true` | GroupDetailEmptyView |
| `.loaded(schedules)` | `false` | 일정 리스트 |
...
### schedulesState
```swift
public var schedulesState: LoadingState<[ScheduleItem]> = .idle
```
- 일정 리스트의 로딩 상태
- `.idle` - 초기 상태
- `.loading` - 로딩 중
- `.loaded([schedules])` - 로드 완료
- `.failed(error)` - 로드 실패
...
public var shouldShowEmptyGroupView: Bool {
  !schedulesState.isLoading && hasNoGroups
}
```
...
```
.internal(.loadSchedulesResponse(.failure(error)))
  ↓
schedulesState = .failed(error)
  ↓
화면: 에러 메시지 + [재시도] 버튼
...
- 재시도 시 `fetchSchedules(groupId)` 재실행
...
state.selectedFilter = .needResponse
  ↓
View에서 filteredSchedules 재계산
  └─ schedules.filter { $0.needsResponse }
```

**필터 종류:**
- `.all` - 모든 일정
- `.needResponse` - 응답이 필요한 일정 (status == "pending")
- `.confirmed` - 확정된 일정 (isConfirmed == true)

### 일정 수락/거절

```
.view(.scheduleAccepted(scheduleId))
  ↓
Effect: 서버에 응답 전송
  ↓
Firestore 업데이트:
...
sequenceDiagram
    ...
    participant PC as ScheduleClient
    participant FS as Firestore

    V->>R: .view(.onAppear)
    R->>R: schedulesState = .loading
    R->>GC: fetchGroups()
    ...
    alt 그룹이 없는 경우
        R->>R: schedulesState = .loaded([])
        R->>V: GroupDetailEmptyView
    else 그룹이 있는 경우
        R->>R: .selectDefaultGroup(groups)
        R->>R: currentGroup = selected
        R->>PC: fetchSchedules(groupId)
        PC->>FS: schedules (where groupId)
        FS-->>PC: schedules[]
        PC-->>R: .loadSchedulesResponse(.success(schedules))
        R->>R: schedulesState = .loaded(schedules)
        R->>V: ScheduleTimelineView
    end
```
