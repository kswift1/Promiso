...
// MARK: - Schedule Response Status UI Extension
...
struct ScheduleCardView: View {
  ...
  private var responseStatus: ScheduleResponseStatus {
    schedule.responseStatus(currentUserId: currentUserId)
  }
        ...
        HStack(spacing: 12) {
          ...
          if !schedule.imageUrls.isEmpty {
            ScrollView(.horizontal, showsIndicators: false) {
              HStack(spacing: 6) {
                ForEach(schedule.imageUrls.prefix(4), id: \.self) { urlString in
                  if let url = URL(string: urlString) {
                    ScheduleItemThumbnail(url: url, size: 64, placeholder: Color.gray.opacity(0.15))
                  }
                }
                if schedule.imageUrls.count > 4 {
                  Text("+\(schedule.imageUrls.count - 4)")
...
private let scheduleViewCalendar = Calendar.current

// MARK: - Compact Day Row (일정 + 캘린더 이벤트 통합)

/// 월간 뷰용 컴팩트 행 (일정과 캘린더 이벤트 모두 표시)
...
struct CompactDayRow: View {
  ...
  private func scheduleRow(_ schedule: ScheduleModel) -> some View {
    let groupColor = groupColorMap[schedule.groupId]
    ...
    Button {
        ...
        VStack(alignment: .leading, spacing: 4) {
          HStack(spacing: 6) {
            ...
            HStack(spacing: 4) {
              ...
              Text(unifiedStatusText(for: schedule))
                .font(.system(size: 12, weight: .medium))
                .foregroundColor(unifiedStatusColor(for: schedule))
            }
          }
          HStack(spacing: 4) {
            Text(schedule.timeRangeText)
      ...
      .background(
        RoundedRectangle(cornerRadius: 6)
          .fill(schedule.responseStatus(currentUserId: currentUserId) == .needResponse
 ...
 private func recurringEventTimeRow(_ event: ExpandedEventInstance) -> String {
  ...
  private func unifiedStatusColor(for schedule: ScheduleModel) -> Color {
    let status = schedule.responseStatus(currentUserId: currentUserId)
    switch status {
    ...
    case .confirmed:
      return schedule.isPast ? Color.pmgray.n500 : Color.pmsuccess.n500
  ...
  private var dayNumber: String {
    String(scheduleViewCalendar.component(.day, from: date))
  }
  ...
  private var isToday: Bool {
    scheduleViewCalendar.isDateInToday(date)
  }
...
#Preview("Schedule Card - Confirmed") {
...
#Preview("Schedule Card - Needs Response") {
  let sampleGroup = GroupModel(
    id: "group1",
    name: "친구들",
    memberIds: ["user1", "user2"],
