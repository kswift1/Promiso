...
// MARK: - Calendar Day Timeline Pager

/// CalendarFeature 전용 3페이지 일간 수평 페이저
struct CalendarDayTimelinePager: UIViewControllerRepresentable {
  let selectedDate: Date
  let prevDayScheduleItems: [CalendarFeature.ScheduleItem]
  let currentDayScheduleItems: [CalendarFeature.ScheduleItem]
  let nextDayScheduleItems: [CalendarFeature.ScheduleItem]
  ...
  let onCreateSchedule: (Date, Date) -> Void
  ...
  func makeUIViewController(context: Context) -> PagerViewController {
    ...
    vc.setupPages(
      ...
      onPastTimeBlocked: onPastTimeBlocked,
  ...
  func updateUIViewController(_ vc: PagerViewController, context: Context) {
    ...
    if coordinator.needsRecenter {
      ...
      vc.updatePages(
        ...
        onPastTimeBlocked: coordinator.pager.onPastTimeBlocked,
  ...
  final class Coordinator: NSObject, UIScrollViewDelegate {
    var pager: CalendarDayTimelinePager
    ...
    init(pager: CalendarDayTimelinePager) {
      self.pager = pager
    ...
    func scrollViewDidEndScrollingAnimation(_ scrollView: UIScrollView) {
      ...
      vc.updatePages(
        ...
        onPastTimeBlocked: pager.onPastTimeBlocked,
  ...
  final class PagerViewController: UIViewController {
    ...
    func setupPages(
      prevDayScheduleItems: [CalendarFeature.ScheduleItem],
      currentDayScheduleItems: [CalendarFeature.ScheduleItem],
      nextDayScheduleItems: [CalendarFeature.ScheduleItem],
      selectedDate: Date,
      ...
      for (index, items) in itemsArrays.enumerated() {
        let dayTimelineView = CalendarDayTimelineView(
          scheduleItems: items,
    ...
    func updatePages(
      ...
      for (index, vc) in pageHostingControllers.enumerated() {
        ...
        vc.rootView = CalendarDayTimelineView(
    ...
    func updateSinglePage(
      at index: Int,
      scheduleItems: [CalendarFeature.ScheduleItem],
      displayDate: Date,
      onScheduleItemTapped: @escaping (CalendarFeature.ScheduleItem) -> Void,
      onEditScheduleItem: ((CalendarFeature.ScheduleItem) -> Void)?,
      onCreatePersonalEvent: @escaping (Date, Date) -> Void,
      ...
      pageHostingControllers[index].rootView = CalendarDayTimelineView(
