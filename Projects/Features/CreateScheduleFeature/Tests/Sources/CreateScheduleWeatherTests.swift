//
//  CreateScheduleWeatherTests.swift
//  GroupFeature
//
//  CreateSchedule.Feature 날씨 힌트 기능 테스트 (Swift Testing + TCA TestStore)
//
//  ## 테스트 대상
...
// MARK: - CreateSchedule Weather Tests

@Suite("CreateSchedule.Feature 날씨 힌트 테스트")
@MainActor
struct CreateScheduleWeatherTests {
  ...
  func setStartDate_noLocation_doesNotFetchWeather() async {
    ...
    await store.send(.view(.setStartDate(forecastDate))) {
      $0.schedule.startAt = forecastDate
      $0.weatherState = .idle
    }
  }
  ...
  func setStartDate_noCoordinates_doesNotFetchWeather() async {
    ...
    state.schedule.location = LocationInfoModel(
      name: "장소명만",
      address: "주소만",
      latitude: nil,
      longitude: nil
  ...
  func setStartDate_farFutureDate_doesNotFetchWeather() async {
    ...
    await store.send(.view(.setStartDate(farFutureDate))) {
      $0.schedule.startAt = farFutureDate
      $0.weatherState = .idle
    }
  }
  ...
  func setStartDate_tenDaysLater_fetchesWeather() async {
    var state = CreateSchedule.Feature.State()
    state.useLocation = true
    state.schedule.location = makeLocation()
    state.schedule.startAt = Date()
    ...
    await store.send(.view(.setStartDate(tenDaysLater))) {
      $0.schedule.startAt = tenDaysLater
    }
    await store.skipReceivedActions()
  ...
  func startDate_oneSecondLater_fetches() async {
    ...
    await store.send(.view(.setStartDate(oneSecondLater))) {
      $0.schedule.startAt = oneSecondLater
    }
    await store.skipReceivedActions()
    #expect(store.state.weatherState.value != nil || store.state.weatherState.isLoading)
  }
