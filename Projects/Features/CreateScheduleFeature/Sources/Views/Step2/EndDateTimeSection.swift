...
struct EndDateTimeSection: View {
  ...
  private var useEndTime: Bool {
    store.state.schedule.endAt != nil
  }

  var body: some View {
    SectionPlaceHolder(
      placeHolderTitle: LocalizedStrings.CreateSchedule.endTime,
      ...
      content: {
        VStack {
          ...
          if useEndTime {
            EndTimePicker(
              startDate: store.schedule.startAt,
              endDate: Binding(
                get: {
                  store.schedule.endAt ?? store.schedule.startAt.addingTimeInterval(7200)
                },
...
struct EndTimePicker: View {
  ...
  private var durationText: String {
    startDate.durationText(to: endDate, prefix: LocalizedStrings.CreateSchedule.totalDurationPrefix)
  }
