...
struct MinimumParticipantsSection: View {
  ...
  private var currentMinimum: Int {
    store.schedule.minimumParticipants
  }

  // 1명 고정 여부 (그룹 최대 인원이 1명인 경우)
  private var isFixedAtOne: Bool {
    store.schedule.group?.maxMembers == 1
  }
  ...
  var body: some View {
    SectionPlaceHolder(placeHolderTitle: LocalizedStrings.CreateSchedule.minimumParticipants) {
  ...
  private var fixedParticipantsView: some View {
    VStack(spacing: 16) {
      VStack(spacing: 8) {
        Text(LocalizedStrings.CreateSchedule.participantsCount(1))
          .font(.system(size: 36, weight: .bold))
          .foregroundColor(.primary)

        Text(LocalizedStrings.CreateSchedule.maxParticipants(1))
          .font(.system(size: 13))
      ...
      HStack(spacing: 12) {
        ...
        Text(LocalizedStrings.CreateSchedule.fixedParticipantsInfo)
          .font(.system(size: 14))
          .foregroundColor(.secondary)
      }
      .padding(16)
  ...
  private var adjustableParticipantsView: some View {
    HStack(spacing: 16) {
      ...
      VStack(spacing: 4) {
        Text(LocalizedStrings.CreateSchedule.participantsCount(currentMinimum))
          .font(.system(size: 36, weight: .bold))
          .foregroundColor(.primary)
          .contentTransition(.numericText())

        Text(LocalizedStrings.CreateSchedule.maxParticipants(maxParticipants))
          .font(.system(size: 13))
          .foregroundColor(.secondary)
      }
      .frame(maxWidth: .infinity)
  ...
  private var infoBox: some View {
    HStack(alignment: .top, spacing: 12) {
      ...
      Text(LocalizedStrings.CreateSchedule.autoConfirmInfo(currentMinimum))
        .font(.system(size: 14))
        .foregroundColor(.primary)
  ...
  private var warningMessage: some View {
    HStack(alignment: .top, spacing: 8) {
      ...
      Text(LocalizedStrings.CreateSchedule.maxParticipantsWarning)
        .font(.system(size: 13))
        .foregroundColor(.secondary)
        .fixedSize(horizontal: false, vertical: true)
