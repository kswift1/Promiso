...
struct GroupListView: View {
  ...
  let groupScheduleCounts: [String: Int]
  let maxActiveSchedules: Int
    ...
    VStack(spacing: 12) {
      ForEach(groups) { group in
        GroupCard(
          ...
          activeScheduleCount: groupScheduleCounts[group.id],
          maxActiveSchedules: maxActiveSchedules
        ) {
          isFocused = false
  ...
  private var emptyStateView: some View {
    VStack(spacing: 16) {
      ...
      Text(LocalizedStrings.CreateSchedule.noGroupsTitle)
        .font(.headline)
        .foregroundColor(.primary)

      Text(LocalizedStrings.CreateSchedule.noGroupsSubtitle)
        .font(.subheadline)
  ...
  private func errorView(error: Error) -> some View {
    VStack(spacing: 16) {
      ...
      Text(LocalizedStrings.CreateSchedule.loadGroupsFailed)
        .font(.headline)
        .foregroundColor(.primary)
    ...
    Button {
      ...
      HStack(spacing: 8) {
        Image(systemName: "plus.circle.fill")
          .font(.system(size: 18))
        Text(LocalizedStrings.CreateSchedule.createNewGroup)
          .font(.headline)
      }
    ...
    Button {
      ...
      HStack(spacing: 8) {
        Image(systemName: "arrow.clockwise")
          .font(.system(size: 18))
        Text(LocalizedStrings.CreateSchedule.retryButton)
          .font(.headline)
      }
      .foregroundStyle(.white)
      .frame(maxWidth: .infinity)
...
struct GroupCard: View {
  ...
  init(
    model: GroupModel,
    isSelected: Bool,
    activeScheduleCount: Int? = nil,
    maxActiveSchedules: Int = 10,
    action: @escaping () -> Void
  ) {
    self.model = model
    self.isSelected = isSelected
    self.activeScheduleCount = activeScheduleCount
    self.maxActiveSchedules = maxActiveSchedules
    self.action = action
  }

  private var isAtLimit: Bool {
    guard let count = activeScheduleCount else { return false }
    return count >= maxActiveSchedules
  }
  ...
  private var disabledReason: String? {
    if hasTooFewMembers {
      return LocalizedStrings.CreateSchedule.noMembersInGroup
    } else if isAtLimit {
      return LocalizedStrings.CreateSchedule.activeSchedulesAtLimit(maxActiveSchedules)
    }
    return nil
  }
...
#Preview("Loaded") {
  ...
  GroupListView(
    ...
    selectedGroupId: "g2",
    groupScheduleCounts: ["g2": 5, "g3": 10], // g3 is at limit
    maxActiveSchedules: 10,
    onGroupSelected: { _ in },
    onRetry: {},
    onCreateGroup: {},
