...
extension CreatePersonalEvent {
  public struct RootView: View {
    ...
    // MARK: - Text Extraction Section

    @ViewBuilder
    private var textExtractionSection: some View {
      // create 모드에서만 표시
      if store.mode == .create {
        VStack(spacing: 0) {
          ...
          Button {
            ...
            HStack {
              ...
              Text(LocalizedStrings.Personal.extractFromText)
                .font(.body)
                .foregroundStyle(Color.pmtext.primary)

              Spacer()

              Image(systemName: store.isTextExtractionExpanded ? "chevron.up" : "chevron.down")
                .font(.system(size: 14))
          ...
          // 펼쳐진 상태
          if store.isTextExtractionExpanded {
            Divider()
            ...
            VStack(spacing: 12) {
              ...
              TextEditor(text: Binding(
                get: { store.extractionText },
                set: { store.send(.view(.extractionTextChanged($0))) }
              ))
              .font(.system(size: 14))
              ...
              HStack(spacing: 8) {
                ...
                Button {
                  ...
                  HStack(spacing: 4) {
                    Image(systemName: "doc.on.clipboard")
                      .font(.system(size: 13))
                    Text(LocalizedStrings.Personal.extractionPaste)
                      .font(.system(size: 14))
                  }
                  .foregroundStyle(Color.pmindigo.n500)
                  .padding(.horizontal, 12)
                ...
                Button {
                  ...
                  HStack(spacing: 4) {
                    ...
                    Text(LocalizedStrings.Personal.extractionExtract)
                  ...
                  .padding(.vertical, 8)
                  .background(
                    store.extractionText.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty || store.isExtracting
                      ? Color.pmgray.n400
                      : Color.pmindigo.n500
                  )
                  .clipShape(RoundedRectangle(cornerRadius: 8))
                ...
                .buttonStyle(.plain)
                .contentShape(Rectangle())
                .disabled(store.extractionText.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty || store.isExtracting)
              }
              ...
              if let error = store.extractionError {
                HStack {
                  ...
                  Spacer()
                  Button {
                    store.send(.view(.dismissExtractionError))
                  } label: {
        ...
        .adaptiveGlassCard()
        .animation(.default, value: store.isTextExtractionExpanded)
      }
    }
