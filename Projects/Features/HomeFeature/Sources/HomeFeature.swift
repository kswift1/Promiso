...
// TCA 1.22.2를 사용한 Home Feature의 완전한 구현
...
/// 오늘의 약속 섹션
private struct TodayPromiseSection: View {
  let store: StoreOf<Home.Feature>
...
/// 다가오는 약속 섹션
private struct UpcomingPromiseSection: View {
  let store: StoreOf<Home.Feature>
  ...
  var body: some View {
    WithPerceptionTracking {
      VStack(alignment: .leading, spacing: 12) {
        ...
        if store.upcomingPromises.isEmpty {
          VStack(spacing: 8) {
...
/// 대기 중인 응답 섹션
private struct PendingResponseSection: View {
  let store: StoreOf<Home.Feature>
      ...
      VStack(alignment: .leading, spacing: 12) {
        ...
        LazyVStack(spacing: 8) {
          ForEach(store.pendingResponses) { response in
            PendingResponseCard(response: response) { id, accept in
              store.send(.respondToProposal(id, accept))
            }
          }
        }
        .padding(.horizontal, 16)
...
private struct PromiseCard: View {
    ...
    Button(action: onTap) {
      HStack(spacing: 12) {
        ...
        VStack(alignment: .trailing, spacing: 4) {
          ...
          if promise.liveActivityStatus == .canStart {
            Button("시작") {
              onTap()
            }
            .font(.caption)
  ...
  private var statusBadge: some View {
    Text(promise.status.title)
      .font(.caption2)
      .fontWeight(.medium)
      .foregroundColor(.white)
      .padding(.horizontal, 6)
      .padding(.vertical, 2)
      .background(promise.status.color)
      .cornerRadius(4)
  }
}

/// 대기 중인 응답 카드 컴포넌트
private struct PendingResponseCard: View {
  let response: Home.PendingResponse
