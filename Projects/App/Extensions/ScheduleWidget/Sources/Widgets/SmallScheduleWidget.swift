...
/// 홈 화면 작은 크기 위젯 (systemSmall) - 다음 일정 1개 표시
struct ScheduleSystemSmallWidget: Widget {
  let kind: String = WidgetKind.systemSmall

  var body: some WidgetConfiguration {
    StaticConfiguration(kind: kind, provider: ScheduleTimelineProvider()) { entry in
      ScheduleSystemSmallWidgetView(entry: entry)
    ...
    .description(LocalizedStrings.Widget.configNextScheduleDescription)
...
struct ScheduleSystemSmallWidgetView: View {
  ...
  private func accessibilityLabel(for item: WidgetScheduleData) -> String {
