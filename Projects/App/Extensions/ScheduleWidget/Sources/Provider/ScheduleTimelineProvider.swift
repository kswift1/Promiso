...
struct ScheduleTimelineProvider: TimelineProvider {
  typealias Entry = WidgetScheduleEntry
  ...
  private func createEntryFromCache() -> Entry {
    guard WidgetDataManager.isLoggedIn() else {
      return Entry(date: Date(), schedules: [], state: .notLoggedIn)
    }

    let allItems = WidgetDataManager.loadAllItems()
    let state: Entry.WidgetState = allItems.isEmpty ? .empty : .loaded
    return Entry(date: Date(), schedules: allItems, state: state)
  }

  private func calculateNextRefresh(items: [WidgetScheduleData]) -> Date {
