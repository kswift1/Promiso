  ...
  var body: some WidgetConfiguration {
    ActivityConfiguration(for: ScheduleActivityAttributes.self) { context in
      ...
      LockScreenBannerView(context: context)
        .widgetURL(AppConstants.Deeplink.url(path: "live/\(context.attributes.scheduleId)"))
...
private let previewAttributes = ScheduleActivityAttributes(
  scheduleId: "preview-123",
...
/// 긴 제목 테스트용
private let previewAttributesLong = ScheduleActivityAttributes(
  scheduleId: "preview-long",
  currentUserId: "user-1",
  emoji: "🎂",
...
/// 1. 초기 상태 - 모두 대기
private let stateInitial = ScheduleActivityAttributes.ContentState(
  trackingDurationMinutes: 30,
  participants: [
...
/// 2. 진행 중 - 일부 출발
private let stateInProgress = ScheduleActivityAttributes.ContentState(
  trackingDurationMinutes: 30,
  participants: [
...
/// 3. 긴급 상태 - 거의 도착
private let stateUrgent = ScheduleActivityAttributes.ContentState(
  trackingDurationMinutes: 30,
  participants: [
...
/// 4. 거의 완료 - 대부분 도착
private let stateAlmostDone = ScheduleActivityAttributes.ContentState(
  trackingDurationMinutes: 30,
  participants: [
...
/// 5. 완료 - 모두 도착
private let stateCompleted = ScheduleActivityAttributes.ContentState(
  trackingDurationMinutes: 30,
  participants: [
    ParticipantState(id: "user-1", name: "나", estimatedArrivalMinutes: 0),
    ParticipantState(id: "user-2", name: "민수", estimatedArrivalMinutes: 0),
...
/// 6. 다양한 진행률
private let stateMixed = ScheduleActivityAttributes.ContentState(
  trackingDurationMinutes: 30,
  participants: [
    ParticipantState(id: "user-1", name: "나", estimatedArrivalMinutes: 0),
