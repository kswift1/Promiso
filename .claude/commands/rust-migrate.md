...
3. `infra/rust-backend/`의 현재 상태 확인 (없으면 Step 7에서 생성)
...
**⚠️ 1:1 복사 방지 체크리스트** (각 분리에 대해 반드시 질문):

```
□ 이 서브컬렉션/필드가 Firestore에서 분리된 이유는? (보안 규칙? 성능? 관행?)
□ 그 이유가 PostgreSQL에서도 유효한가?
□ 같은 테이블 컬럼으로 합치면 문제가 되는가?
□ 별도 테이블이 필요하다면 PostgreSQL 고유의 이유가 있는가? (N:M 관계, 독립 생명주기 등)
```

유저에게 Before/After 스키마 + 분리/합침 근거를 보여주고 확인받는다.
