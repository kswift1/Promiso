...
### 병렬 실행 원칙
- **독립적인 작업은 무조건 병렬**로 에이전트를 실행한다 (하나의 메시지에 여러 Agent 호출)
- 파일 의존성이 없는 작업들은 동시 실행, 의존성이 있으면 배치를 나눠 순차 실행
- 탐색(Explore)과 구현(implementer)도 독립적이면 병렬 실행 가능

### 에이전트 권한 장애 대응
- 에이전트가 Write/Edit 권한 문제로 실패하면, **직접 처리로 넘어가지 말고 유저에게 보고**한다
- 보고 내용: 실패 원인, 해결 방안 (settings.json 권한 추가 등)
- 유저 승인 후 권한 문제를 해결하고, 에이전트를 재실행한다
- 권한 해결이 불가능한 경우에만 유저 승인 하에 직접 처리
