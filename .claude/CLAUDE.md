...
**모든 개발 작업은 반드시 다음 6단계를 순차적으로 진행하세요.**

```
작업 공간 설정 [📋 유저 확인] → 탐색 → 계획 [🔒 유저 승인] → 구현 → 검증 [📋 유저 확인] → 커밋 [🔒 유저 승인]
```
...
| 단계 | 개입 수준 | 설명 |
|------|----------|------|
| 0. 작업 공간 설정 | 📋 **유저 확인** | 브랜치 네이밍, 베이스 브랜치, 작업 공간 방식 확인 |
...
| 규모 | 기준 | 워크플로우 | 예시 |
|------|------|----------|------|
| **S** (단순) | 1~2 파일, 10줄 이내 | 작업 공간→탐색→구현→검증→커밋[🔒] | 타이포 수정, 한 줄 버그 |
| **M** (보통) | 3~5 파일, 1 모듈 | 전체 6단계 | 기능 추가, 버그 수정 |
| **L** (대형) | 6+ 파일, 복수 모듈 | 전체 6단계 + 팀 병렬 | 대규모 기능, 리팩터링 |
...
### 0️⃣ 작업 공간 설정 (Workspace Setup) — 🟢 자동 진행

**담당**: team-lead (orchestrator)
**목적**: 이슈별 독립적인 작업 공간 확보
**개입**: 📋 **유저 확인** — 브랜치 네이밍, 베이스 브랜치, 작업 공간 방식을 확인받고 진행

**실행 방법**:
```
1. 현재 상태 확인
   - git status (미커밋 변경사항 존재 여부)
   - git branch --show-current (현재 브랜치)

2. 작업 공간 결정
   ┌─────────────────────────────────────────────────────┐
   │ 현재 폴더에 진행 중 작업 없음 → 브랜치 생성 후 작업  │
   │ 현재 폴더에 진행 중 작업 있음 → Worktree 생성        │
   │ 빌드 환경 격리 / 장기 병렬    → 레포 새로 클론       │
   └─────────────────────────────────────────────────────┘

3. 브랜치 생성
   - base: 최신 release 브랜치에서 분기
   - 네이밍: fix/{이슈내용}, feat/{기능명}, refactor/{대상}

4. Worktree 생성 시
   - git worktree add ../Promiso-{브랜치명} -b {브랜치명} {base}
   - make setup 실행 (프로젝트 생성 + 워크스페이스 생성)

5. 레포 클론 시
   - git clone {remote} ../Promiso-{브랜치명}
   - make setup 실행 (프로젝트 생성 + 워크스페이스 생성)
```
...
**사용 가능한 플러그인 에이전트**:
- `Explore` - 코드베이스 탐색 (6단계 워크플로우 1단계)
...
[orchestrator가 6단계 워크플로우를 3번 반복]
