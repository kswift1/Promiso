...
.PHONY: setup ensure-config feature remove-feature color \
        rust-run rust-test rust-build rust-env rust-env-clean \
...
# GCP Secret Manager에서 DB URL 로드 → infra/rust-backend/.env 생성
# 사용법: make rust-env            (기본: dev)
#        make rust-env ENV=stage
#        make rust-env ENV=prod
rust-env:
	@ENV=$${ENV:-dev}; \
	PROJECT=promiso-$$ENV; \
	echo "📥 $$PROJECT → infra/rust-backend/.env"; \
	{ \
	  echo "# Auto-generated by 'make rust-env ENV=$$ENV'"; \
	  echo "# GCP Secret Manager가 Single Source of Truth. 수동 편집 금지."; \
	  echo "DATABASE_URL=$$(gcloud secrets versions access latest --secret=DATABASE_URL --project=$$PROJECT)"; \
	  echo "DATABASE_POOL_URL=$$(gcloud secrets versions access latest --secret=DATABASE_POOL_URL --project=$$PROJECT)"; \
	} > infra/rust-backend/.env
	@echo "✅ infra/rust-backend/.env 생성 완료"

# 생성된 .env 파일 제거
rust-env-clean:
	@rm -f infra/rust-backend/.env
	@echo "🧹 infra/rust-backend/.env 제거"
